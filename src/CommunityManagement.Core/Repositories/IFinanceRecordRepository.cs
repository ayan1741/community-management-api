...
public interface IFinanceRecordRepository
{
    ...
    Task<MonthlyFinanceTotals> GetMonthlyTotalsAsync(Guid orgId, int year, int month, CancellationToken ct = default);
    Task<IReadOnlyList<MonthlyFinanceTotals>> GetAnnualTotalsAsync(Guid orgId, int year, CancellationToken ct = default);
    Task<IReadOnlyList<CategoryBreakdownItem>> GetCategoryBreakdownAsync(Guid orgId, string type, int year, int month, CancellationToken ct = default);
    Task<IReadOnlyList<MonthAmountItem>> GetExpenseTrendAsync(Guid orgId, int months, CancellationToken ct = default);
    ...
    Task<decimal> GetDuesCollectedAsync(Guid orgId, int year, int month, CancellationToken ct = default);
    Task<IReadOnlyList<decimal>> GetAnnualDuesCollectedAsync(Guid orgId, int year, CancellationToken ct = default);
